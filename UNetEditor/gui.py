...
class ImageEditorGUI(ipywidgets.HBox):
    def __init__(self,update_event=None,reset_event=None):
        ...
        self.colorbar = self.fig1.colorbar(self.im1,cax=self.cax)
...
class UNetPredictionGUI(ipywidgets.VBox):
    def __init__(self,modeldic,pred_event=None):
        ...
        self.ax[0].set_title("Kerr micrograph")
        self.ax[1].set_title("Predicted label")
        self.ax[0].grid(False)
        self.ax[1].grid(False)
        cbar = self.fig.colorbar(plt.cm.ScalarMappable(norm=matplotlib.colors.BoundaryNorm([0,1,2,3],3), 
						cmap=matplotlib.colors.ListedColormap([(0,0,1),(0,1,0),(1,0,0)])),
						ax=self.ax[1], ticks=[0.5, 1.5, 2.5],orientation="horizontal")
        cbar.set_ticklabels(['Background', 'Defects', 'Skyrmions'])
        cbar1 = self.fig.colorbar(self.im1,ax=self.ax[0],orientation="horizontal")
        #cbar.set_ticklabels(['Background', 'Defects', 'Skyrmions'], rotation=90)
