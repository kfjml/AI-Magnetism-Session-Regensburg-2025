...
class MaskAnalysis:
    def __init__(self,nbins=40):
        ...
        {"ix":7,"quant":r"Skyrmion hole area [$\mathrm{pixel}^2$]","title":"Skyrmion hole area","unit":r"$\mathrm{pixel}^2$","qminmax_standard":(0.5,"img_s_max"),"slider_type":"log"}]
        ...
        for i in range(len(quant)):
            ...
            quant[i]["qcb"] = qcb = self.fig.colorbar(qim,cax=qax[2],label=quant[i]["quant"])#,location='left')
...
class PosAnalysis:
    def __init__(self):
        ...
        self.cb2 = plt.colorbar(self.sc2,cax=cax2,label=r"$\Psi_6$")
        ...
        self.cb3 = plt.colorbar(self.sc3,cax=cax3,label=r"$\Psi_4$")
    ...
    def get_datatable(self):
        ...
        return pd.DataFrame({"Skyrmion-Skyrmion distance [pixel]":self.lo[:,4],
                             r"$\Psi_6$ [1]":self.lo[:,2],r"$\Psi_4$ [1]":self.lo[:,3]},index=self.lo[:,5].astype(int))
